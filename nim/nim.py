...
class NimAI():
    ...
    def best_future_reward(self, state):
        ...
        for action in Nim.available_actions(state):
            if self.get_q_value(state, action) > best_fut_reward:
                best_fut_reward = self.get_q_value(state, action)
        
        return best_fut_reward
    ...
    def choose_action(self, state, epsilon=True):
        ...
        if epsilon:
            ...
            # (1 - epsilon) for best_action
            # epsilon spread equally among all other actions
